...
def repository_changed():
    ...
    url = 'https://raw.githubusercontent.com/juliand7gj/sd2018b-exam1/' + pull_id + '/packages.json'
    ...
    for i in packages_json:
        ...
        for j in commands:
                install_packages = install_packages + " " + j
    text = Connection('192.168.131.22').run('sudo yum install --downloadonly --downloaddir=/var/repo' + install_packages)
    logging.debug(text)
    result = {'command_return': 'Bien'}
    return result
